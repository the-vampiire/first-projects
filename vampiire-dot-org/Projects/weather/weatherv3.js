...
// JQuery shortcuts
    ...
    // Unused (available as needed in future)
    time_div = $('#time'),
    date_div = $('#date'),
...
// Weather Option Switches
    ...
    // Flickr Background Feature (default: Off)
    // Queries Flickr to produce a background image local to the current location
    var flickr = flickr_switch.prop('checked');

    flickr_switch.click(function(){
        flickr = flickr_switch.prop('checked');
    });


// Location Switches

    location_options.on('shown.bs.collapse', function(){
        $('#vampiire_codes')[0].scrollIntoView(false);
    });
...
// Secure Location function
    ...
    // https and user accepts location services
        ...
        function get_location_secure() {
            ...
            // flips secure switch off, IP switch on, resets get_display function with new location setting
                secure_switch.prop('checked', false);
                    secure = secure_switch.prop('checked');
                insecure_switch.prop('checked', true);
                    insecure = insecure_switch.prop('checked');
    ...
    function get_flickr(lat, lon) {
        ...
        $.getJSON('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=97d8e60e16e9237ae774020d6e8713a2&' +
            ...
            if (photo === undefined || photo.farm === undefined) {
                ...
                flickr_switch.prop('checked', false);
                flickr = flickr_switch.prop('checked');
            }
        ...
        // if on mobile device
            if(mobile){
                ...
                setTimeout(function(){
                    instructions_row.show();
                    instructions.fadeIn(1000, function(){
                        instructions.html('<i class="fa fa-hand-pointer-o fa-2x" aria-hidden="true"></i> swipe for more images!');
                        instructions.animate({
                            fontSize: '1.5em'
                        }, 'slow');

                    });
                    instructions_row.fadeOut(9000, function(){
                        instructions.empty();
                    });
                }, 3500);
                ...
                // remove previous event listeners
                    jumbotron.off('swiperight');
                    jumbotron.off('swipeleft');


                // add event listeners
                   jumbotron.on('swipeleft', function(){
                       cycle_forward(flickr_bg);
                   });

                   jumbotron.on('swiperight', function(){
...
// Set Background function
    function set_background(arr){
        ...
        $('#background').fadeOut(500, 'swing', function(){
            $('#background').css('background-image', arr[0]);
        });

        $('#background').fadeIn(500, 'swing');
...
// Cycle Background function
    ...
    // Used for cycling stock background images
        // var i;
        //
        // function cycle_forward(arr){
        //     if(flickr){
...
// Display weather functions
    ...
    function display_weather(data) {
        ...
        var temp_f = current.temp_f, /* use for displaying *F <span>&#8457;</span> */
            ...
            // Celsius
            temp_c = current.temp_c, /* use for displaying *C <span>&#8451;</span> */
    ...
    // Stock Background Arrays
        ...
        // Daytime Switch
            if(is_day === 1){
                switch(condition_code){
                    // sunny
                    case 1000:
                        set_background(day_bg[0]);
                        break;
                    // partly cloudy
                    case (1003):
                    case (1006):
                        set_background(day_bg[1]);
                        break;
                    // overcast
                    case (1009):
                        set_background(day_bg[2]);
                        break;
                    // fog
                    case (1030):
                    case (1135):
                        set_background(day_bg[3]);
                        break;
                    // rainy
                    case (1063):
                    case (1087):
        ...
        // Night Time Switch
                ...
                switch(condition_code){
                    ...
                    // night sky
                    case 1000:
                        set_background(night_bg[0]);
                        break;
                    // cloudy
                    case (1003):
                    case (1006):
                    case (1009):
                        set_background(night_bg[1]);
                        break;
                    // fog
                    case (1030):
                    case (1135):
                        set_background(night_bg[2]);
                        break;
                    // rainy
                    case (1063):
                    case (1087):
